...
class GenPlate_NewVer:
    def __init__(self, fontCh_file, fontEng_path_file, NoPlates):
        ...
        self.text_component = {7: {'seg': (3, 4), 'dash_size': (10, 10)},
                               6: {'seg': (3, 3), 'dash_size': (10, 10)},
                               5: {'seg': (2, 3), 'dash_size': (10, 20)}}

        self.fontC_small = ImageFont.truetype(fontCh_file, 30, 0)
    ...
    def addElectricString(self, img):
        ch_base = round(img.shape[1] / 3) + 5
        gap = round(ch_base / 3 - self.ch_font_width)
        for i in range(3):
            img[0:0 + self.ch_font_height, ch_base:ch_base + self.ch_font_width] = GenCh(self.fontC_small, electric_string[i], self.ft_color, self.ch_font_width, self.ch_font_height)
    ...
    def addExhaust(self, img):
        ...
        if r(2) is 0:
            ...
            x = img.shape[1] - (exhaust.shape[1] + r(50) + 5)   # right
        y = r(10) + 5
    ...
    def draw(self, val):
        ...
        seg_front = self.text_component[str_length]['seg'][0]
        seg_rear = self.text_component[str_length]['seg'][1]
        ...
        # num font
        base += self.text_component[str_length]['dash_size'][1] + 5  # shift bacause the '-'
    ...
    def genDash(self, img, length):
        dash_height = self.text_component[length]['dash_size'][0]
        dash_width = self.text_component[length]['dash_size'][1]
        # start position
        x = 0
        y = 0
        if length == 7:
            x = round(img.shape[1] / 2) - 30
            y = round(img.shape[0] / 2)
        elif length == 6:
            x = round((img.shape[1] - dash_width) / 2)
            y = round(img.shape[0] / 2)
        elif length == 5:
            x = round(img.shape[1] / 2) - 40
            y = round(img.shape[0] / 2)
    ...
    def genPlateString(self, val):
        plateStr = ""
        seg_front = self.text_component[val]['seg'][0]

        for cpos in range(val):
            if cpos < seg_front:
                plateStr += chars[10 + r(24)]  # english
            else:
    ...
    def genBatch(self, batchSize, pos, charRange, img_size, text_len=-1, transpose=False, color_code='BGR'):
        ...
        for i in range(batchSize):
            # random gen 5~7 text r(3) is 0, 1 or 2
            if text_len == -1:
                plateStr = self.genPlateString(r(3) + 5)
            else:
